...
bool samescc(int a,int b,vector<vector <int> > scc_list)
{
    for(int i=0;i<scc_list.size();i++)
    {
        for(int j=0;j<scc_list[i].size();j++)
        {
            if(a==scc_list[i][j])
            {
                for(int k=0;k<scc_list[i].size();k++)
                    if(b==scc_list[i][k])
                        return true;
...
int main(int argc, char* argv[]) {
    ...
    for(int i=0;i<kernels.size();i=i+2)
    {
